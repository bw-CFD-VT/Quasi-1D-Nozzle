...
int main()
{
    ...
    string grid_ID = "Check.txt";    // Grid ID -> "_x.txt"
    ...
    Exact_Solution(imax,x_cell_center,Area_cell_center,M_exact,rho_exact,u_vel_exact,p_exact,T_exact, grid_ID, V_exact,U_exact);
    //-------------------------------------------------------------------------------------------------------------------------//
    ...
    {  
        ...
        double d_t = *std::min_element(dt.begin(),dt.end()); //Global Time Step
        ...
        for (int i = 0; i<imax; i++)
        {
            for (int j = 0; j<3; j++)
            {
                Residual[i][j] = ((F[i+1][j]+d[i+1][j])*Area_interface[i+1])-((F[i][j]+d[i][j])*Area_interface[i])
                                      -SourceTerm[i][j]*dx;

                U_cell_center[1][i][j] = U_cell_center[0][i][j]-(d_t/(Area_cell_center[i]*dx))*Residual[i][j];
            }
        }
        //----------------------------------------------------------------------------------//    
        ...
        if (counter%100 == 0)
        {
            residual_norm_file(residualnormfile,counter,imax,L2_norm_residual,grid_ID);
            mach_file(machfile,counter,imax,M_cell_center,grid_ID);
            prim_variable_file(rhofile,0,counter,imax,V_cell_center,grid_ID);
            prim_variable_file(ufile,1,counter,imax,V_cell_center,grid_ID);
            prim_variable_file(pressfile,2,counter,imax,V_cell_center,grid_ID);
            prim_variable_file(tempfile,3,counter,imax,V_cell_center,grid_ID);
        }
        //---------------------------------------------------------------------------------//
        ...
        for (int i = 0; i<imax; i++)
        {
            ...
            M_cell_center[i] = V_cell_center[0][i][1]/a[i];
        }
        //---------------------------------------------------------------------------------//
        ...
        //---------------------- Check Convergence ----------------------------------------//
        if(L2_norm_residual[0] < convergence_criteria &&
           L2_norm_residual[1] < convergence_criteria &&
           L2_norm_residual[2] < convergence_criteria) convergence = true;
        //---------------------------------------------------------------------------------//
    ...
    if (Case_Flag == 1)
    {
        ...
        error_norm_file(errornormfile_U,imax,L2_norm_error,grid_ID);

        L2_Norm(imax,DE_V,L2_norm_error);
        error_norm_file(errornormfile_V,imax,L2_norm_error,grid_ID);
    }
    //---------------------------------------------------------------------------------//
};